...
export const setupLevelingCommand = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((option) =>
      option.setName("enabled").setDescription("Enable or disable leveling system").setRequired(true),
    )
  ...
  async execute(interaction) {
    try {
      ...
      log.success(`Leveling system ${enabled ? "enabled" : "disabled"} in ${interaction.guild.name}`)
    } catch (error) {
